...
def test_model_attributes():
    ...
    Checks for the presence of key variables and parameters in the Aliev-Panfilov 0D model.
    """
    model = AlievPanfilov0D(dt=0.01, stimulations=[])

    assert 'u' in model.variables, "Model should have variable 'u'"
    assert 'v' in model.variables, "Model should have variable 'v'"

    assert 'a' in model.parameters, "Model should have parameter 'a'"
    assert 'k' in model.parameters, "Model should have parameter 'k'"
    assert 'eps' in model.parameters, "Model should have parameter 'eps'"
    assert 'mu1' in model.parameters, "Model should have parameter 'mu1'"
    assert 'mu2' in model.parameters, "Model should have parameter 'mu2'"
    

def test_model_run():
    """
    Test the model run for a short simulation.
    Runs the Aliev-Panfilov 0D model with a predefined stimulation protocol and checks
    ...
    model = prepare_model(AlievPanfilov0D, dt=0.01, curr_dur=0.1, curr_value=2.0, t_prebeats=t_prebeats)
